...
import { AccountParser } from './types';
...
export class Bid {
  key: StringPublicKey;
  amount: BN;
  bidder_token : StringPublicKey;
  bidder_pot_token : StringPublicKey;
  constructor(args: { key: StringPublicKey; amount: BN; bidder_token : StringPublicKey; bidder_pot_token : StringPublicKey; }) {
    this.key = args.key;
    this.amount = args.amount;
    this.bidder_token = args.bidder_token;
    this.bidder_pot_token=args.bidder_pot_token;
  }
}
...
export class AuctionDataExtended {
  ...
  constructor(args: {
    ...
    this.name = args.name;
    this.reward_size=args.reward_size;
  }
}
  ...
  [
    CreateAuctionArgs,
    {
      ...
      fields: [
        ...
        ['reward_size',{kind: 'option', type:'u64' }],
      ],
    },
  ],
  [
  ...
  [
    Bid,
    {
      ...
      fields: [
        ['key', 'pubkeyAsString'],
        ['amount', 'u64'],
        ['bidder_token', 'pubkeyAsString'],
        ['bidder_pot_token', 'pubkeyAsString'],
      ],
    },
  ],
  [
...
// export async function endAuctionWithResource(
//   resource : StringPublicKey,
//   creator : StringPublicKey,
//   instructions : TransactionInstruction[],
// ){
...
export async function placeBidV2(
  ...
  resource: StringPublicKey,
  amount: BN,
  prev_bidder : StringPublicKey,
  prev_bidder_token : StringPublicKey,
  prev_bidder_pot_token : StringPublicKey,
  instructions: TransactionInstruction[],
) {
  const auctionProgramId = programIds().auction;
  ...
  const keys = [
    ...
    {
      pubkey: toPublicKey(prev_bidder),
      isSigner: false,
      isWritable: true,
    },
    {
      pubkey: toPublicKey(prev_bidder_token),
      isSigner: false,
      isWritable: true,
    },
    {
      pubkey: toPublicKey(prev_bidder_pot_token),
      isSigner: false,
      isWritable: true,
    },
  ];
